...
```{r}
regions <- st_read("../FIJI/FIJI_admin.shp")
...
#Dave has opened up the regions folder and selected the following columns with the names in the brackets (changed to lower caps in previous code)
regions <- regions %>% 
  select(uid, id_0, name_0, id_1, name_1, name_2)
```
...
leaflet() %>% 
  addTiles() %>% 
  addPolygons(data=regions_4326, popup=~name_2, label=~name_1)
```
